...
<form:form modelAttribute="tbMemVO" name="detailForm" id="detailForm" >
	...
	<table class="bd_tbl bd_tbl_view">
		...
		<tbody>
			...
			<tr>
				<th scope="row"><label for="jobGbCd">직업</label></th>
				<td class="left" colspan="3">
					<ul class="chk_area">
					<c:forEach var="codelist" items="${codelist}">
						<c:if test="${codelist.gbCd == 'plt009'}">
							<li><input onclick="return(false);" type="radio" name="jobGbCd" value="${codelist.code }" id="${codelist.codeNm }" <c:if test="${codelist.code eq tbMemVO.jobGbCd }">checked</c:if>> <label for="${codelist.codeNm }">${codelist.codeNm }</label></li>
						</c:if>
					</c:forEach>
					</ul>
				</td>
	...
	<div class="bd_btn_area right">
		...
		<c:if test="${tbMemVO.code eq '0002' }">								
			<a href="/admin/mem/memList.do" class="btn_black btn_list" >목록</a>
		</c:if>
		<c:if test="${tbMemVO.code eq '0003' }">
			<c:if test="${tbMemVO.comType eq 'C'}">
				<a href="/admin/mem/memCList.do" class="btn_black btn_list">목록</a>
			</c:if>
			<c:if test="${tbMemVO.comType eq 'D'}">
				<a href="/admin/mem/memDList.do" class="btn_black btn_list">목록</a>
			</c:if>
		</c:if>
	</div>
</form:form>