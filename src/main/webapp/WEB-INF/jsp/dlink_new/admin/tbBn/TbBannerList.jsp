...
<%
 /**
  * @Class Name : TbPopupList.jsp
  * @Description : TbPopup List 화면
		...
		<table class="bd_tbl center">
			...
			<tbody>
				...
				<c:forEach var="result" items="${resultList}" varStatus="vs">
					...
					<!-- <td >
						<button type="button" onclick="javascript:seqEdit('down', 10, 88);" class="btn_st mini line_gray">▲</button>
						<button type="button" onclink="javascript:seqEdit('up', 10, 88);"class="btn_st mini line_gray">▼</button>
					</td> -->
				<!-- 리스트 이미지 -->
					<td>
						<img src="/imagefile/preview.do?fileSeq=${result.fileSeq }"  style="width:140px; height:60px">
					</td>
					<td><a href="/admin/tbBn/updateTbPopupView.do?popSeq=${result.popSeq }">${result.title }</a></td>	<!-- 제목/이미지태그  -->
					<td>
