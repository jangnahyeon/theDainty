...
<%
 /**
  * @Class Name : TbCodeRegister.jsp
  * @Description : TbCode Register 화면
...
<c:set var="registerFlag" value="${empty tbCodeVO.code ? '등록' : '수정'}"/>			
...
<!-- validator:javascript formName="tbCodeVO" staticJavascript="false" xhtml="true" cdata="false"/ -->

<script type="text/javaScript" language="javascript" defer="defer">

$(function(){
	<c:if test="${registerFlag eq '수정'}">
		<c:if test="${tbCodeVO.useYn eq 'Y' }">
			$("#useYn_Y").attr("checked", true);
		</c:if>
		<c:if test="${tbCodeVO.useYn ne 'Y' }">
...
function fn_egov_selectList() {
   	document.getElementById("detailForm").action = "<c:url value='/admin/tbCode/TbCodeList.do'/>";
...
function fn_egov_delete() {
	if( !confirm( "삭제하시겠습니까?" ) ) {
   	...
   	document.getElementById("detailForm").action = "<c:url value='/admin/tbCode/deleteTbCode.do'/>";
   	document.getElementById("detailForm").submit();		
}
...
function fn_egov_save() {	
	...
	if(!codeNm) {
  	...
  	frm.action = "<c:url value="${registerFlag == '등록' ? '/admin/tbCode/addTbCode.do' : '/admin/tbCode/updateTbCode.do'}"/>";
    frm.submit();
...
<form:form modelAttribute="tbCodeVO" name="detailForm" id="detailForm" >
<div id="content_pop">
	<!-- 타이틀 -->
	<%-- <div id="title">
		<ul>
	...
	<table class="bd_tbl bd_tbl_view">
		...
		<c:if test="${registerFlag == '수정'}">
			<tr>
				<th>구분코드</th>
				<td>
					<form:input path="gbCd" cssClass="txt" readonly="true" maxlength="10"/>
		...
		<tr>
			<th>코드</th>
			<td>
				<form:input path="code" cssClass="txt" readonly="true" placeholder="코드는 자동으로 생성됩니다." maxlength="10"/>
		...
		<tr>
			<th>코드명</th>
			<td>
				<form:input path="codeNm" cssClass="txt" maxlength="20"/>
