...
<c:set var="registerFlag" value="${empty searchVO.goodsNo || 0 eq searchVO.goodsNo ? '등록' : '수정'}"/>	
...
function fn_egov_add(){
	...
	frm = document.getElementById("detailForm");
  	...
  	frm.action = "<c:url value="${registerFlag == '등록' ? '/admin/goods/addGoodsAct.do' : '/admin/tbBn/updateTbBanner.do'}"/>";
    frm.submit();
}
...
<input type="hidden" name=momId value="${adminLoginVO.memId }"/>
<input type="hidden" name="goodsNo" value="${searchVO.goodsNo }"/>
</c:if>
...
<div id="content_pop">
	...
	<table class="bd_tbl bd_tbl_view">
		...
		<tr>
			<th scope="row"> 상품명<span class="tbl_star">*</span></th>
			<td colspan="3" class="left"><input type="text" name="goodsNm" id="goodsNm" value="${searchVO.goodsNm }"></td>
		</tr>
		<tr>
			<th scope="row">색상<span class="tbl_star">*</span></th>
			<td class="left"><input type="color" name="color" id="color" value="${searchVO.color }"></td>
			<th scope="row">사이즈<span class="tbl_star">*</span></th>
			<td class="left"><input type="text" name="size" id="size" value="${searchVO.size }"></td>
		</tr>
		<tr>
			<th scope="row">원가<span class="tbl_star">*</span></th>
			<td><input type="text" name="unitPrice" id="unitPrice" value="${searchVO.unitPrice }"></td>
			<th scope="row">판매가<span class="tbl_star">*</span></th>
			<td><input type="text" name="sellPrice" id="sellPrice" value="${searchVO.sellPrice }"></td>
		</tr>
		<tr>
		...
		<tr>
			<th scope="row">구매링크<span class="tbl_star">*</span></th>
			<td colspan="3"><input type="text" name="link" id="link" value="${searchVO.link }"></td>
		</tr>
	</table>
  </div>
