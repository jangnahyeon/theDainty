...
.green .fc-daygrid-event-dot {
	...
	border: 3px solid #37d872 !important;
}

.blue .fc-daygrid-event-dot {
	border: 3px solid #3788d8 !important;
}
	   ...
	   windowResize: function(arg) {
	    ...
	    moreLinkClick: "popover",
	      eventDidMount: function(info) {
	          var tooltip = new Tooltip(info.el, {
	            ...
	            title: "<div id='test'>" + info.event.extendedProps.description + "</div>" +
	            		...
	            		"<div id='test3'>" + info.event.title + "</div>",
	            placement: 'top',
	            trigger: 'hover focus',
	            html: true,
	            container: 'body',
 	    ...
 	    events:[
		    	<c:forEach var="TbSisulRsvVO" items="${resultList}">
				    	{
		    		<c:if test="${TbSisulRsvVO.acceptYn == 'W' || TbSisulRsvVO.acceptYn == 'Y'}">
 				    	<c:if test="${TbSisulRsvVO.acceptYn == 'W'}">
 				    		<c:if test="${TbSisulRsvVO.comNm != ''}">
 				    			title: '(' + '가예약중' + ')' +'<c:out value="${TbSisulRsvVO.comNm}" />',
 				    		</c:if>
				    		<c:if test="${TbSisulRsvVO.comNm == '' || TbSisulRsvVO.comNm == null}">
				    			title: '(' + '가예약중' + ')' +'<c:out value="${TbSisulRsvVO.memId}" />',
				    		</c:if>	
				    	</c:if>
					    <c:if test="${TbSisulRsvVO.acceptYn == 'Y'}">
				    		title: '(' + '예약완료' + ')' + '<c:out value="${TbSisulRsvVO.comNm}" />',
				    	</c:if>
				    		...
				    		<c:if test="${TbSisulRsvVO.rsvGbCd == '0002'}">
			    				className: 'green'
			    			</c:if>
					</c:if>
				    	},
		...
		<div class="contents">			
			<div class="innerwrap">	
				...
				<div class="booking_guide" id="booking_guide">
					<ol>
						<li id="focc"><p>먼저 전화로 예약 가능한 일정을 문의해주세요.</br>T. 064-805-3399</p></li>
						<li><p>지정된 날짜를 클릭하여 예약을 진행해주세요.</p></li>
						<li><p>예약자 전화번호로 예약 확정 문자메세지를 보내 드립니다.</p></li>
					</ol>
				</div>
				
				<h3 id="foc">예약현황</h3>
				<div class="right colorNotice" style="right:100px;position:absolute;font-size:15px;">
					<span style="color:#3788d8;">●</span>중소기업 제품 촬영 지원   <span style="color:#37d872;">●</span>셀프 스튜디오 이용
				</div>
