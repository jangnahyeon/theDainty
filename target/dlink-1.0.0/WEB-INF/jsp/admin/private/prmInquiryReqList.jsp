...
function fn_egov_select(loginId,registerFlag) {
    ...
    document.getElementById("listForm").registerFlag.value = registerFlag;
   	document.getElementById("listForm").action = "<c:url value='/admin/private/prmReqDetail.do'/>";
...
function fn_egov_link_page(pageNo){
	...
	document.getElementById("listForm").action = "<c:url value='/admin/private/prmInquiryRequestList.do'/>";
   	document.getElementById("listForm").submit();
}
...
<form:form modelAttribute="searchVO" name="listForm" id="listForm" method="post">
	...
	<table class="bd_tbl bd_biz center" >
		...
		<c:if test="${empty resultList }"><tr><td colspan="6" class="center" >등록된 글이 없습니다.</td></tr></c:if>
		<c:forEach var="result" items="${resultList}" varStatus="status">
			<tr>
				...
				<td class="listtd">
						...
						<c:if test="${result.acceptYn eq 'Y'}">승인</c:if>&nbsp;</td>
			</tr>
		</c:forEach>
	</table>
