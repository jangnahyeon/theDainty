...
function multi_image_delete( val){
	...
	$.ajax({
		url : "/front/TPtplInfo/deleteTPtplInfoImg.do",
		data : formData ,
		processData : false,
		contentType : false,
		async : false,
			...
			<div class="contents">
				...
				<section class="tab_content">
						...
						<tbody>
							...
							<tr>
								...
								<td>
									<div class="group_tel">
										...
										<select id="data_div" name="ptplPart" onchange="javascript:selectPA('new');">
											<option value="none">선택</option>
											<c:forEach var="codeListPA" items="${codeListPA}" varStatus="vs">
												<option value="${codeListPA.code}" <c:if test="${resultList.ptplPart eq codeListPA.code }">selected </c:if> >${codeListPA.codeNm}</option>
											</c:forEach>
										</select>
										<!-- </div> -->
										...
										<input type="hidden" value="${resultList.ptplYouhaeng}" name="youhaengCode"/>
										<!-- </div> -->
									</div>
								</td>
							</tr>
							<%-- <tr>
								...
								<td>
									...
									<input type="hidden" value="${resultList.ptplType}" name="arrPtplType"/>
							...
							<tr>
								...
								<td>
									...
									<input type="hidden" value="${resultList.ptplStyle}" name="arrPtplStyle"/>
									<!-- <p class="mt10 f_size14">※ 최대 5개까지만 입력 가능합니다.</p> -->
								</td>
							</tr>
							<tr>
								...
								<td>
									...
									<input type="hidden" value="${resultList.addOption}" name="arrAddOption"/>
									<!-- <p class="mt10 f_size14">※ 최대 5개까지만 입력 가능합니다.</p> -->
								</td>
							</tr> --%>
							<tr>
								...
								<td class="">									
									<textarea cols="5" rows="8" id="data_cont" name="concept" maxlength="200"/>${resultList.concept }</textarea>
							...
							<tr>
								...
								<td>
									...
									<div id="multi_file_attch" class="multi_file_attch" >
										<c:if test="${not empty resultList.multiSeq }">
										<c:forTokens items="${resultList.multiSeq }" delims="," var="multiSeq">
											<li id="${multiSeq }" style='width:75%;margin-bottom:5px;'>
											<i class="main_img"><img src="/imagefile/preview.do?fileSeq=${multiSeq }" alt="" style="width:100px; height:auto; padding-top:20px"></i>
											<%-- <a href='/imagefile/download.do?file_no=${resultList.fileSeq }' class="mt_image_file">${resultList.fileNm }</a> --%>
											<a class="tbl-btn" href="javascript:multi_image_delete('${multiSeq }');">삭제</a></li><br/>
										</c:forTokens>
										</c:if>
									</div>
								</td>
					...
					<div class="bd_btn_area center">
						<button type="button" id="btnPrevStep" class="btn bl_gray" onclick="javascript:location.href='/front/portMagmt/portInquiryList.do'">취소</button>
						<button type="button" id="save_btn" class="btn bg_gray_dark" onclick="javascript:go_modify();">수정</button>
					</div>
				</section>
				</form:form>
			</div>
...
function selectPA(type) {
	var codePA = $("select[name=ptplPart]").val();
	var ptplYouhaeng = $("input[name=youhaengCode]").val(); 
	...
	$.ajax({
	    ...
	    success: function(data){
	    	...
	    	$.each(arr, function(idx, val) {
	    		if(ptplYouhaeng == val.code && type == "old") {
		    		$("select[name=ptplYouhaeng]").append("<option value='" + val.code + "' selected>" + val.codeNm + "</option>");
	    		} else {
...
/* function selectPY() {
	...
	if(codePY == null) {
		codePY = $("input[name=youhaengCode]").val();
	}
	...
	var arrType = $("input[name=arrPtplType]").val().split(',');
	var arrStyle = $("input[name=arrPtplStyle]").val().split(',');
	var arrAddOption = $("input[name=arrAddOption]").val().split(',');
	...
	if(arrType != null && arrType != '' && arrType != 'undefined') {
		for (var nArrCnt in arrType) {
	         $("input[name=ptplType][value="+arrType[nArrCnt]+"]").prop("checked",true);
		}
	}
	...
	if(arrStyle != null && arrStyle != '' && arrStyle != 'undefined') {
		for (var nArrCnt in arrStyle) {
	        $("input[name=ptplStyle][value="+arrStyle[nArrCnt]+"]").prop("checked",true);
		}
	}
	$.ajax({
		url : "/front/portMagmt/selectCodeListJason.do",
	...
	if(arrAddOption != null && arrAddOption != '' && arrAddOption != 'undefined') {
		for (var nArrCnt in arrAddOption) {
	        $("input[name=addOption][value="+arrAddOption[nArrCnt]+"]").prop("checked",true);
		}
	}
} */
