...
function go_search() {
	document.getElementById("listForm").action = "<c:url value='/admin/dBank/dBankList.do'/>";
   	document.getElementById("listForm").submit();
} 
	...
	<form:hidden path="searchKeyword2" />
						...
						<div class="fr">
							<fieldset class="search_form">
								...
								<div class="bd_sel_search">
									...
									<form:input path="searchKeyword" onkeypress="enterkey()" onkeyup="javascript:enterkey();" id="searchKeyword"/>
					...
					<ul class="lst_thum row01">
						<c:forEach var="result" items="${resultList}" varStatus="vs">
							<li>
								<a href="/admin/dBank/dBankDetail.do?imgNo=${result.imgNo }" onclick="">
									<span class="info_poster"><i><img src="/imagefile/preview.do?fileSeq=${result.fileSeq }" alt="${result.imgNm }"></i></span>								
									<strong>${result.regDt }<i>${result.imgNm }</i></strong>
