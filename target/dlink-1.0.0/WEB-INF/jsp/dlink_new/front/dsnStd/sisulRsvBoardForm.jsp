			...
			<div class="innerwrap">	
				...
				<div class="bgcont_box icon_reserve">
					<ul>
						...
						<!-- 수정 -->
						<li>※ 예약 가능한 날짜는 오늘 날짜보다 최소 7일 이후에 가능합니다. (주말 포함)</li>
...
<!-- 					수정
									...
									<div class="gourp_inp ">        
										<div class=" item_filter">
											<div class="selec_area"><span class="select_btn">개수 선택 </span>
												...
												<select id="prodCnt" class="prodCnt" name="prodCnt"><option>개수 선택</option></select>
											</div>
										</div>
									</div>
								</div>
								...
								<div class="wirte_content_inner">
									...
									<c:forEach var="codelist" items="${codelist}">
										<c:if test="${codelist.gbCd == 'plt004'}">
											<li><input type="radio" name="purposeCd" id="${codelist.codeNm}" value="${codelist.codeNm}"><label for="${codelist.codeNm}" class="">${codelist.codeNm}</label></li>
										</c:if>
									</c:forEach>
									</ul>
								</div>
						...
						<div class="wirte_container">
							<div class="wirte_tit">
								<label for="wr_name">중소상공인확인서 파일첨부<b>*</b><strong class="blind">필수</strong></label>
							</div>
					...
					<div class="pop_agreement_view">
						...
						<div class="pop_inner">
							<div class="agree_wrap">
								...
								<div class="agree_index">
									<ul class="list_type01">
										<li>1. 예약 후 부득이한 상황으로 취소를 원하시는 경우, 최소 48시간 전에 취소 바랍니다.</li>
										<li>2. 이용 당일 취소하시는 경우, 이용일 기준 30일 동안 이용에 제한이 있습니다.</li>
										<li>3. 1개 업체 기준 월 최대 2회, 일 최대 4시간 이용 가능합니다.</li>
										...
										<li>6. 스튜디오 촬영 도중 발생한 사고와 관련하여 본 시설 운영자의 고의 또는 중과실이
										있는 경우를 제외하고는 인적, 물적 손해에 대한 배상 책임을 지지 않습니다.</li>
										<li>7. 촬영 결과물은 jpg, png 파일 제공을 기본으로 하며 메일을 통해 제공됩니다.</li>
										<li>8. 촬영 결과물은 7일간 보관 후 폐기됩니다.</li>
										<li>9. 결과물의 저작권은 제주 디자인 스튜디오와 공동으로 소유하며, 본 스튜디오에서 촬영한 사진,
											 영상 등의 콘텐츠는 제주 디자인 스튜디오의 홍보에 사용될 수 있습니다.
										</li>
									</ul>
								</div>
							</div>
...
function sisulRsv_check(){
	...
	var smBizFileNm = document.getElementById("smBizFileNm");
...
/* 	if(prod2.value == ""){
	...
	if($("input:radio[name=purposeCd]:checked").length < 1){
	...
	if(!confirm("시설예약을 하시겠습니까?")){
		...
		$("#title").attr("value",  rsvYear.value + "년" + rsvMonth.value + "월" + rsvDay.value + "일" + "(" + wk + ")" + comNm);
