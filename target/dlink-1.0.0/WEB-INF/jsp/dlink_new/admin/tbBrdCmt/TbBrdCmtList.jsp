...
<c:forEach var="result" items="${resultList }">
	...
	<li>
		<em>관리자<span>${regDay }<span>${regTime }</span></span></em>
		<div class="replyCont${result.cmtSeq}">
			<%-- ${result.cont } --%>
			${fn:replace(result.cont, newLineChar, '<br/>') }
		</div>
		<div class="replyUpdate${result.cmtSeq}" style="display:none">
			<textarea id="cont${result.cmtSeq}" >${result.cont }</textarea>
		</div>
		<p class="btn_area_reply">
			<a href="javascript:fn_cmt_del_act('${result.cmtSeq }');" class="btn_st bg_gray">삭제</a>
			<a href="#" onclick="javascript:fn_cmt_update('${result.cmtSeq}');" class="btn_st line_gray updateBtn"><span>수정</span></a>
			<a href="#" onclick="javascript:fn_cmt_update_act('${result.cmtSeq}');" class="btn_st line_gray saveBtn" style="display:none"><span>저장</span></a>
		</p>
	</li>
</c:forEach>
